...
### Changed
* `PreComputedRoundCPU::init` derives Poseidon's round constants once per field and caches them; `HashCPU::hash` was re-deriving them on every call. Adds an `F: 'static` bound to `init` [#527](https://github.com/midnightntwrk/midnight-zk/pull/527)
