...
impl Relation for AtalaJsonECDSA {
    ...
    fn circuit(
        ...
        // Check Name.
        let name = Self::get_property(std_lib, layouter, &json, &parsed_json, 1, NAME_LEN)?;
        Self::assert_str_match(std_lib, layouter, &name, VALID_NAME)?;

        // Check birth date.
        let birthdate =
            Self::get_property(std_lib, layouter, &json, &parsed_json, 2, BIRTHDATE_LEN)?;
        Self::assert_date_before(std_lib, layouter, &birthdate, MAX_VALID_DATE)?;

        // Get holder public key.
        let x = Self::get_property(std_lib, layouter, &json, &parsed_json, 3, COORD_LEN)?;
        let y = Self::get_property(std_lib, layouter, &json, &parsed_json, 4, COORD_LEN)?;
