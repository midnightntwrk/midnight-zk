...
//     ecc::{
//         curves::CircuitCurve,
//         foreign::{nb_foreign_ecc_chip_columns, ForeignEccChip,
// ForeignEccConfig},     },
//     field::{
//         decomposition::{
...
// Msm, SelfEmulation,         VerifierGadget,
//     },
// };
// use midnight_proofs::{
//     circuit::{Layouter, SimpleFloorPlanner, Value},
//     plonk::{create_proof, keygen_pk, keygen_vk_with_k, prepare, Circuit,
// ConstraintSystem, Error},     poly::{kzg::KZGCommitmentScheme,
// EvaluationDomain},     transcript::{CircuitTranscript, Transcript},
// };
// use rand::rngs::OsRng;
//
...
// (domain, cs, vk_repr)     // We use a simple application function that
// increases a counter.     prev_state: Value<F>,
...
//
//     let advice_columns: Vec<_> = (0..nb_advice_cols).map(|_|
// meta.advice_column()).collect();     let fixed_columns: Vec<_> =
// (0..nb_fixed_cols).map(|_| meta.fixed_column()).collect();
...
//         let pow2_config = Pow2RangeChip::configure(meta,
// &advice_columns[1..NB_ARITH_COLS]);
//         P2RDecompositionChip::configure(meta, &(native_config.clone(),
// pow2_config))     };
//
//     let base_config = FieldChip::<F, CBase, C, NG>::configure(meta,
// &advice_columns);     let curve_config =
//         ForeignEccChip::<F, C, C, NG, NG>::configure(meta, &base_config,
// &advice_columns);
//
...
//     ) -> Result<(), Error> {
//         let native_chip = <NativeChip<F> as
// ComposableChip<F>>::new(&config.0, &());         let core_decomp_chip =
// P2RDecompositionChip::new(&config.1, &(K as usize - 1));         let
// scalar_chip = NativeGadget::new(core_decomp_chip.clone(),
// native_chip.clone());         let curve_chip = {
// ForeignEccChip::new(&config.2, &scalar_chip, &scalar_chip) };         let
// poseidon_chip = PoseidonChip::new(&config.3, &native_chip);
//
//         let verifier_chip = VerifierGadget::new(&curve_chip, &scalar_chip,
// &poseidon_chip);
//
//         let self_vk_name = "self_vk";
//         let (self_domain, self_cs, self_vk_value) = &self.self_vk;
//         let assigned_self_vk: AssignedVk<S> =
// verifier_chip.assign_vk_as_public_input(             &mut layouter,
//             self_vk_name,
//             self_domain,
//             self_cs,
//             *self_vk_value,
...
//         let next_state = scalar_chip.add_constant(&mut layouter, &prev_state,
// F::ONE)?;         scalar_chip.constrain_as_public_input(&mut layouter,
// &next_state)?;
//
//         // Witness a proof and an accumulator that ensure the validity of
// `prev_state`.         let prev_acc = {
...
//         };
//
//         let id_point = curve_chip.assign_fixed(&mut layouter,
// C::identity())?;
//
//         let assigned_pi = [
...
//         .collect::<Vec<_>>();
//
//         // Verify a witnessed proof that ensures the validity of
// `prev_state`.         // The proof is valid iff `proof_acc` satisfies the
// invariant.         let mut proof_acc = verifier_chip.prepare(
//             &mut layouter,
//             &assigned_self_vk,
...
//         )?;
//
//         // If `prev_state` is genesis, we allow the prover to change the
// (probably         // invalid) accumulator by a default accumulator that
// satisfies the invariant.         let is_genesis = scalar_chip.is_zero(&mut
// layouter, &prev_state)?;         let is_not_genesis = scalar_chip.not(&mut
// layouter, &is_genesis)?;
//
...
//
//         // Accumulate the `proof_acc` with the previous witnessed
// accumulator.         // `next_acc` will satisfy the invariant iff both
// `proof_acc` and `prev_acc` do.         let mut next_acc =
// AssignedAccumulator::<S>::accumulate(             &mut layouter,
//             &verifier_chip,
//             &scalar_chip,
//             &poseidon_chip,
//             &[proof_acc, prev_acc],
//         )?;
//
//         // Finally, collapse the resulting accumulator and constraint it as
// public.         next_acc.collapse(&mut layouter, &curve_chip, &scalar_chip)?;
//
...
//
//     // This trivial accumulator must have a single base and scalar of F::ONE,
// and     // the base has to be the default point of C. This is because when
// parsing     // an empty proof, our transcript gadget places a default point
// on every     // `read_point`. Note that the `base` is left untouched on
// during the     // handling of genesis, because `scale_by_bit` only modifies
// the scalars.     //
//     // On the other hand, the scalar has to be F::ONE because it is the value
//     // obtained after a `collapse` (the last step before constraining the acc
// as     // a public input).
...
//             &[C::default()],
//             &[F::ONE],
//             &fixed_base_names.iter().map(|name| (name.clone(),
// F::ZERO)).collect(),         ),
//     );
//
...
//         let proof = {
//             let mut transcript =
// CircuitTranscript::<PoseidonState<F>>::init();             create_proof::<
//                 F,
...
//                 &mut transcript,
//             )
//             .unwrap_or_else(|_| panic!("Problem creating the {i}-th IVC
...
//             let mut transcript =
// CircuitTranscript::<PoseidonState<F>>::init_from_bytes(&proof);             
// let dual_msm =                 prepare::<F, KZGCommitmentScheme<E>,
// CircuitTranscript<PoseidonState<F>>>(                     &vk,
//                     &[&[C::identity()]],
//                     &[&[&public_inputs]],
//                     &mut transcript,
//                 )
...
//         prev_acc = acc.clone();
//
//         // If `acc` satisfies the invariant and `proof` is valid, we know
// that `state`         // must be valid. We can asset the validity of both at
// the same time by         // accumulating them first.
