...
impl Relation for AtalaJsonECDSA {
    ...
    fn circuit(
        ...
        // Assign the verification data as public input.
        // let data = std_lib.assign_many(layouter, &instance.transpose_array())?;
        let payload = std_lib.assign_many(layouter, &witness.transpose_array())?;

        // Decode and parse Base64 JSON.
        let json = b64_chip.decode_base64(layouter, &payload, false)?;
...
// -----------------------------------------------
impl AtalaJsonECDSA {
    /// Creates an AtalaJsonECDSA witness from A JWT encoded Atala credential.
    ...
    ///  * signature
    ///
    /// Returns the body. For reference: <https://auth0.com/docs/secure/tokens/json-web-tokens/json-web-token-structure>
