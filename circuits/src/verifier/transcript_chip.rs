//! Transcript chip module, for in-circuit Fiat-Shamir.
...
mod tests {
    ...
    fn configure(
        ...
        TranscriptGadget::configure_from_scratch(meta, &instance_column)
    }
    ...
    impl Circuit<F> for TestCircuit {
        ...
        fn synthesize(
            ...
            let mut transcript_chip = TranscriptGadget::new_from_scratch(&config);
            transcript_chip.init_with_proof(&mut layouter, Value::unknown())?;

            TranscriptGadget::load_from_scratch(&mut layouter, &config);

            let assigned_scalars = transcript_chip
                .native_chip
                .assign_many(&mut layouter, &self.scalars.transpose_array())?;

            let assigned_points = transcript_chip
                .curve_chip
                .assign_many(&mut layouter, &self.points.transpose_array())?;

            for i in 0..(SIZE / 2) {
                transcript_chip.common_scalar(&mut layouter, &assigned_scalars[i])?;
                transcript_chip.common_point(&mut layouter, &assigned_points[i])?;
            }

            let challenge_1 = transcript_chip.squeeze_challenge(&mut layouter)?;
            transcript_chip
                .native_chip
            ...
            let challenge_2 = transcript_chip.squeeze_challenge(&mut layouter)?;
            transcript_chip
                .native_chip
    ...
    fn test_transcript_gadget() {
        ...
        let k = 12;
        let public_inputs = vec![vec![challenge_1, challenge_2]];
