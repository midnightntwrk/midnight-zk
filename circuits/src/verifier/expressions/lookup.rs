...
pub(crate) fn lookup_expressions<S: SelfEmulation>(
    ...
    // Helper polynomial constraint: h(X) * ∏_j(f_j(X) + β) = Σ_j ∏_{k≠j}(f_k(X) +
    // β) This ensures the helper polynomial has the correct structure for LogUp
    // soundness. Note: This must hold everywhere (as a polynomial identity),
    // not just at active rows.
    ...
    // h(X) * product - sum_partial_rounds = 0
    let id_1 = {
    ...
    // LogUp accumulator constraint: Z(ωX) * (S(X) + β) = (Z(X) + h(X)) * (S(X) + β)
    // - m(X)
    let id_2 = {
        let left = {
            scalar_chip.mul(
