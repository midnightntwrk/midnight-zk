...
use midnight_proofs::{
    ...
    poly::{fflonk::FflonkScheme, EvaluationDomain, PolynomialLabel},
};
...
/// `T_MAX_LOG = 0` singleton path (algebraically identical to KZG, so the
/// existing GWC machinery in `circuits/src/verifier/kzg.rs` works
/// unchanged). Bumping the const generic here is part of the in-circuit
/// fflonk follow-up (bundle pre-expansion in-circuit).
type VerifyingKey<S> =
    plonk::VerifyingKey<<S as SelfEmulation>::F, FflonkScheme<<S as SelfEmulation>::Engine, 0>>;
