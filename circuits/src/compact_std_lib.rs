...
use keccak::{
    ...
    packed_chip::{AbsorbedBlock, PackedChip, PackedConfig, PACKED_ADVICE_COLS, PACKED_FIXED_COLS},
};
...
pub struct ZkStdLib {
    ...
    // Third-party chips.
    keccak_chip: Option<PackedChip<F>>,
    blake2b_chip: Option<Blake2bChip>,
...
impl ZkStdLib {
    ...
    pub fn new(config: &ZkStdLibConfig, max_bit_len: usize) -> Self {
        ...
        let keccak_chip = config.keccak_config.as_ref().map(PackedChip::new);
...
impl ZkStdLib {
    ...
    pub fn keccak(
        &self,
        layouter: &mut impl Layouter<F>,
        input: &AbsorbedBlock<F>,
    ) -> Result<<PackedChip<Fq> as Keccackf1600Instructions<Fq>>::Digest, Error> {
        *self.used_keccak.borrow_mut() = true;
        let chip = self.keccak_chip.as_ref().expect("ZkStdLibArch must enable keccak");
        let state = chip.initialize_and_absorb(layouter, input)?;
        ...
        *self.nb_public_inputs.borrow_mut() =
        ...
        if let Some(keccak_chip) = zk_std_lib.keccak_chip {
            if *zk_std_lib.used_keccak.borrow() {
                keccak_chip.load_table(&mut layouter)?;
            }
        }
