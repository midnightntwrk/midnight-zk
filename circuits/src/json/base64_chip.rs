                ...
                let decoded_outs: Vec<Value<F>> = b64_input
                    ...
                    .map(|vs| {
                        vs[0].value().zip(vs[1].value()).map(|(c0, c1)| {
                            let v0 = super::table::decode_char(c0.0 as char) as u64;
                            let v1 = super::table::decode_char(c1.0 as char) as u64;
                            F::from(v0 * (1 << 6) + v1)
                        })
                    })
                    .collect();
...
Deep roots are not reached by the frost.
 - J.R.R. Tolkien, 1954";
    ...
    fn test_b64chip_w_padding() {
        ...
        let circuit = TestCircuit::<Fp>::new(b64_input, output, true, false);
        let circuit_bad = TestCircuit::<Fp>::new(b64_input_bad, output, true, false);
