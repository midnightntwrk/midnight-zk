...
use crate::{
    ...
    parsing::{
        native_automaton::{
            ChunkAutomaton, AUTOMATON_CHUNK_PADDING, AUTOMATON_FINAL_PADDING,
...
/// and each of the following `AUTOMATON_MAX_CHUNK_SIZE` pairs counts one input
/// column and one output column.
pub const NB_AUTOMATON_COLS: usize = 1 + 2 * AUTOMATON_MAX_CHUNK_SIZE;
    ...
    fn load(&self, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        layouter.assign_table(
            ...
            |mut table| {
                let mut offset = 0;
                let mut add_entry =
                    |source: F, letter: [F; AUTOMATON_MAX_CHUNK_SIZE], target: F, marker:[F; AUTOMATON_MAX_CHUNK_SIZE]| -> Result<(), Error> {
                        table.assign_cell(
                            || "t_source",
                        ...
                        for i in 0..AUTOMATON_MAX_CHUNK_SIZE {
                            table.assign_cell(
                                || format!("t_letter_{}", i),
                ...
                add_entry(F::ZERO, [F::ZERO; AUTOMATON_MAX_CHUNK_SIZE], F::ZERO, [F::ZERO; AUTOMATON_MAX_CHUNK_SIZE])?;
                ...
                for automaton in self.config.automata.iter() {
                    ...
                    for state in automaton.1.final_states.iter() {
                        add_entry(*state, [F::from(AUTOMATON_FINAL_PADDING as u64); AUTOMATON_MAX_CHUNK_SIZE], F::ZERO, [F::ZERO; AUTOMATON_MAX_CHUNK_SIZE])?
                    }
                }
                Ok(())
            },
    ...
    F: PrimeField + Ord,
{
    ...
    fn apply_one_transition(
        ...
        letters: &[AssignedNative<F>; AUTOMATON_MAX_CHUNK_SIZE],
        ...
        let letters_value = letters
            ...
            .collect::<Value<Vec<_>>>()
            .map(|v| <[F; AUTOMATON_MAX_CHUNK_SIZE]>::try_from(v).unwrap());
        ...
        for i in 0..AUTOMATON_MAX_CHUNK_SIZE {
            ...
            if i <= automaton.chunk_size {
                // Adds the output marker, except those for chunk padding. The final-state
                // padding is not filtered here but at the end of the `parse` function.
                markers.push(output)
            }
        }
    ...
    fn preprocess_parsing_input(
        ...
        final_padding: &AssignedNative<F>,
        chunk_padding: &AssignedNative<F>,
    ) -> Result<Vec<[AssignedNative<F>; AUTOMATON_MAX_CHUNK_SIZE]>, Error> {
        let chunk_size = self.config.automata[automaton_index].chunk_size;
        Ok((input.iter().map(AssignedNative::from).chunks(chunk_size))
            .into_iter()
            .map(|c| {
                c.pad_using(chunk_size, |_| final_padding.clone())
                    .pad_using(AUTOMATON_MAX_CHUNK_SIZE, |_| chunk_padding.clone())
                    .collect::<Vec<_>>()
                    .try_into()
                    .unwrap()
            })
    ...
    pub fn parse(
        ...
        // Padding used to fill transitions blanks when the chunk size of the current
        // automaton is lower than `AUTOMATON_MAX_CHUNK_SIZE`.
        let chunk_padding: AssignedNative<F> = self
            .native_gadget
            .assign_fixed(layouter, F::from(AUTOMATON_CHUNK_PADDING as u64))?;
        ...
        let input_chunked =
            self.preprocess_parsing_input(automaton_index, input, &final_padding, &chunk_padding)?;
    ...
    F: PrimeField + Ord,
{
    ...
    fn configure_from_scratch(
        ...
        let automata = FxHashMap::from_iter(
            [
                (Automaton::hard_coded_example0().chunked(1)),
                (Automaton::hard_coded_example0().chunked(2)),
                (Automaton::hard_coded_example1().chunked(1)),
                (Automaton::hard_coded_example1().chunked(2)),
            ]
            .into_iter()
            .enumerate(),
        );
        ...
        F: PrimeField + Ord,
    {
        ...
        fn synthesize(
            ...
            println!(">> [test] About to parse an automaton with index {}, which contains {} transitions, and {} final states. Inputs will be divided in chunks of size {}",
                ...
                automaton_chip.config.automata[&self.automaton_index].chunk_size
            );
            let parsed_output =
