...
use crate::parsing::automaton::{Marker, ALPHABET_MAX_SIZE};

/// Maximal number of bytes that may be read at the same time by an automaton.
pub(crate) const AUTOMATON_MAX_CHUNK_SIZE: usize = 2;

/// Bytes as used by the automaton chip. They may be regular bytes, or different
/// flavours of padding.
#[derive(Debug, Clone, Copy, Hash, PartialEq, Eq, PartialOrd, Ord)]
pub enum AutomatonByte {
    FinalPadding,
    ChunkPadding,
    Byte(u8),
}
...
/// Value used to represent empty bytes representing padding cells after an
/// incomplete chunk is parsed.
pub(crate) const AUTOMATON_CHUNK_PADDING: usize = ALPHABET_MAX_SIZE + 1;

#[cfg(test)]
/// Transitions reading multiple bytes in a row.
type PartiallyChunkedTransitions =
    FxHashMap<usize, FxHashMap<Vec<AutomatonByte>, (usize, Vec<Marker>)>>;

/// Transitions reading `AUTOMATON_MAX_CHUNK_SIZE` bytes in a row.
type ChunkedTransitions = FxHashMap<
    usize,
    FxHashMap<
        [AutomatonByte; AUTOMATON_MAX_CHUNK_SIZE],
        (usize, [Marker; AUTOMATON_MAX_CHUNK_SIZE]),
    >,
>;

/// The native-field version of `ChunkedTransition`.
type NativeTransitions<F> =
    BTreeMap<F, BTreeMap<[F; AUTOMATON_MAX_CHUNK_SIZE], (F, [F; AUTOMATON_MAX_CHUNK_SIZE])>>;
...
pub struct ChunkAutomaton {
    ...
    pub nb_states: usize,
    /// Number of bytes that can be read by automaton transition (1 is a regular
    /// automaton).
...
impl Automaton {
    ...
    /// # Panics
    ///
    /// If `chunk_size > AUTOMATON_MAX_CHUNK_SIZE` or `chunk_size == 0`.
    pub(crate) fn chunked(&self, chunk_size: usize) -> ChunkAutomaton {
        assert!(
            0 < chunk_size && chunk_size <= AUTOMATON_MAX_CHUNK_SIZE,
            "Automata have to read between 1 and {} bytes at a time",
            AUTOMATON_MAX_CHUNK_SIZE
        );
        let mut transitions = (self.transitions.iter())
            .map(|(source, successors)| {
                let new_successors = (successors.iter())
                    .map(|(byte, (target, marker))| {
                        (vec![AutomatonByte::Byte(*byte)], (*target, vec![*marker]))
                    })
        ...
        for current_chunk_size in 2..=AUTOMATON_MAX_CHUNK_SIZE {
            ...
            for (source, successors) in &transitions {
                for (bytes, (target, markers)) in successors {
                    if current_chunk_size > chunk_size {
                        // We are beyond the chunk size. Therefore, filling with padding bytes.
                        increment(
                            *source,
                            bytes,
                            AutomatonByte::ChunkPadding,
                            *target,
                            markers,
                            Marker::from(0),
                        )
...
impl ChunkAutomaton {
    ...
    /// `ChunkAutomaton`.
    fn chunk_and_pad_input(&self, input: &[u8]) -> Vec<[AutomatonByte; AUTOMATON_MAX_CHUNK_SIZE]> {
        (input.iter().map(|b| AutomatonByte::Byte(*b)).chunks(self.chunk_size))
            .into_iter()
            .map(|c| {
                c.pad_using(self.chunk_size, |_| AutomatonByte::FinalPadding)
                    .pad_using(AUTOMATON_MAX_CHUNK_SIZE, |_| AutomatonByte::ChunkPadding)
                    .collect::<Vec<_>>()
                    .try_into()
                    .unwrap()
            })
    ...
    fn get(
        &self,
        state: usize,
        letters: &[AutomatonByte; AUTOMATON_MAX_CHUNK_SIZE],
...
impl ChunkAutomaton {
    ...
    fn encode_transition<F: PrimeField>(
        source: usize,
        letters: &[AutomatonByte],
        target: usize,
        markers: &[Marker],
    ) -> (
        F,
        [F; AUTOMATON_MAX_CHUNK_SIZE],
        F,
        [F; AUTOMATON_MAX_CHUNK_SIZE],
    ) {
        (
            ...
            letters
                .iter()
                .map(|b| match b {
                    AutomatonByte::FinalPadding => F::from(AUTOMATON_FINAL_PADDING as u64),
                    AutomatonByte::ChunkPadding => F::from(AUTOMATON_CHUNK_PADDING as u64),
                    AutomatonByte::Byte(b) => F::from(*b as u64),
                })
                .collect::<Vec<_>>()
                .try_into()
                .unwrap(),
    ...
    F: PrimeField + Ord,
{
    fn from(automaton: &ChunkAutomaton) -> Self {
        ...
        Self {
            ...
            chunk_size: automaton.chunk_size,
