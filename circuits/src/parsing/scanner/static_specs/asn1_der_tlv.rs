...
pub(super) fn spec_asn1_der_tag() -> Regex {
    ...
    let tag_short =
        Regex::byte_from((0u8..=30).flat_map(class_form_prefix)).output(&|b| Some(b as usize));
    ...
    let tag_continuation = Regex::byte_from(0x80u8..=0xFF).output(&|b| Some((b & 0x7F) as usize));
    // Final tag byte (bit 7 = 0): last fragment of the tag number.
    // Outputs the byte value (bit 7 is already 0).
    let tag_long_last = Regex::byte_from(0x00u8..=0x7F).output(&|b| Some(b as usize));
...
/// Controls which outputs the DER length automaton produces.
#[derive(Clone, Copy)]
enum DerLengthMarking {
    ...
    /// encoding size via a simple sum over the output buffer.
    TotalBytes,
...
fn spec_asn1_der_length_with(m: DerLengthMarking) -> Regex {
    ...
    let short = Regex::byte_from(0x00u8..=0x7F).output(&|b| Some(if value { b as usize } else { 1 }));
    let long_first =
        Regex::byte_from(0x81u8..=0xFF).output(&|b| total.then(|| (b & 0x7F) as usize + 1));
    let value_byte = Regex::any_byte().output(&|b| value.then_some(b as usize));
