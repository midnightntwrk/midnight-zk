...
use crate::{
    ...
    instructions::{
        ArithInstructions, AssignmentInstructions, ControlFlowInstructions, RangeCheckInstructions,
    },
    ...
    fn index_and_pack_sequence(
        ...
        (sequence.iter().enumerate())
            .map(|(i, byte)| {
                ...
                self.native_gadget
                    .linear_combination(layouter, &terms, constant * shift)
            })
            .collect()
    }
...
mod test {
    ...
    fn test_check_bytes() {
        ...
        check_bytes_test(true, 10, (full, sub, 6), ("world", "orl", 1), true);
    }
}
