...
/// subgroup.
/// However, since in most use cases we want to ensure the point is in the subgroup,
/// the implementation of `InnerValue` and `AssignmentInstructions` require subgroup
/// membership.
/// To use arbitrary points of the curve there are equivalent functions that explicitly
/// state the abscence of this subgroup membership check.
...
/// TODO: Consider adding these in in `EccInstructions`.
impl<C: EdwardsCurve> EccChip<C> {
