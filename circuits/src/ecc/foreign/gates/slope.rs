        ...
        meta.create_gate("Foreign-field EC lambda slope", |meta| {
            ...
            let native_id = &cond
                * (&sign - Expression::from(1) + &sign * sum_exprs::<F>(&bs, &qys)
                    ...
                    - sum_exprs::<F>(&bs, &qxs)
                    + sum_exprs::<F>(&bs, &pxs)
                    - karatsuba_bilinear_sum::<F>(&bs2, &lambdas, &qxs)
                    + karatsuba_bilinear_sum::<F>(&bs2, &lambdas, &pxs)
                ...
                .map(|((mj, vj), vj_bounds)| {
                    ...
                    &cond
                        * (&sign - Expression::from(1) + &sign * sum_exprs::<F>(&bs_mj, &qys)
                            ...
                            - karatsuba_bilinear_sum::<F>(&bs2_mj, &lambdas, &qxs)
                            + karatsuba_bilinear_sum::<F>(&bs2_mj, &lambdas, &pxs)
