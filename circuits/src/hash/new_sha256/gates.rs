...
pub fn Sigma_0_gate<F: PrimeField>(
    ...
    [sprdd_evn_11a, sprdd_evn_11b, sprdd_evn_10]: [Expression<F>; 3],
    [sprdd_odd_11a, sprdd_odd_11b, sprdd_odd_10]: [Expression<F>; 3],
    ...
    let sprdd_evn =
        linear_combination_pow4([21, 10, 0], [&sprdd_evn_11a, &sprdd_evn_11b, &sprdd_evn_10]);

    // 4^21 * ~Odd.11a + 4^10 * ~Odd.11b + ~Odd.10
    let sprdd_odd =
        linear_combination_pow4([21, 10, 0], [&sprdd_odd_11a, &sprdd_odd_11b, &sprdd_odd_10]);
...
/// Decompose 11-11-10 gate.
pub fn decompose_11_11_10_gate<F: PrimeField>(
    [limb_11a, limb_11b, limb_10]: [Expression<F>; 3],
    output: Expression<F>,
) -> Vec<(&'static str, Expression<F>)> {
    // (2^21 * Limb.11a + 2^10 * Limb.11b + Limb.10) - output
    let decomposition_check =
        linear_combination_pow2([21, 10, 0], [&limb_11a, &limb_11b, &limb_10]) - output;

    vec![("11-11-10 decomposition check", decomposition_check)]
}
