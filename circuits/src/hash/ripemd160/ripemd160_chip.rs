...
impl<F: CircuitField> RipeMD160Chip<F> {
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let [R_11a, R_11b, R_10] =
                    res_val.map(|v| u32_to_limbs_be(v, [11u8, 11, 10])).transpose_array();
    ...
    fn assign_sprdd_11_11_10(
        ...
        let [evn_11a, evn_11b, evn_10] =
            evn_val.map(|v| u32_to_limbs_be(v, [11u8, 11, 10])).transpose_array();

        let [odd_11a, odd_11b, odd_10] =
            odd_val.map(|v| u32_to_limbs_be(v, [11u8, 11, 10])).transpose_array();
