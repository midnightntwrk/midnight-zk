...
use crate::hash::sha256::{
    table11::{
        spread_table::PostponedSpreadVar, SpreadVar, SpreadWord, StateWord, Table11Assignment,
    },
...
pub fn get_state_addition_digest_row(st_row: StateRow) -> usize {
    let step = SUBREGION_MAIN_WORD;
    match st_row {
        StateRow::A => step - 1,
        StateRow::B => 2 * step - 1,
        StateRow::C => 3 * step - 1,
...
impl CompressionConfig {
    pub(super) fn decompose_abcd<F: PrimeField>(
        ...
        // Postpone c_lo, c_mid, c_hi
        postponed.push(Box::new(c_lo.clone()));
        postponed.push(Box::new(c_mid.clone()));
        postponed.push(Box::new(c_hi.clone()));
    ...
    pub(super) fn decompose_efgh<F: PrimeField>(
        ...
        // postpone a_lo, a_hi, b_hi
        postponed.push(Box::new(a_lo.clone()));
        postponed.push(Box::new(a_hi.clone()));
        postponed.push(Box::new(b_hi.clone()));
    ...
    pub(super) fn decompose_iv_a<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<RoundWordA<F>, Error> {
        let row = get_decompose_a_row(RoundIdx::Init);

        let a = IV[0];
        // no need to look up the two pairs of (dense_half, spread_half) returned below,
        // since only the sum (spread_lo + spread_hi) will be used and the sum
        // is checked to be well formed in decompose_abcd.
        let (dense_halves, spread_halves) = self.assign_word_halves_fixed(region, row, a)?;
        let a_pieces = self.decompose_abcd(region, row, Value::known(a), postponed)?;
    ...
    pub(super) fn decompose_a<F: PrimeField>(
        ...
        region: &mut Region<'_, F>,
        round_idx: RoundIdx,
        assigned_a: &RoundWordDense<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<RoundWordA<F>, Error> {
        let row = get_decompose_a_row(round_idx);

        let a_value = assigned_a.value();
        // no need to look up the two pairs of (dense_half, spread_half) returned below,
        // since only the sum (spread_lo + spread_hi) will be used and the sum
        // is checked to be well formed in decompose_abcd.
        let (dense_halves, spread_halves) = self.assign_word_halves(region, row, a_value)?;
        if let RoundIdx::Init = round_idx {
            region.constrain_equal(dense_halves.0.cell(), assigned_a.0.cell())?;
            region.constrain_equal(dense_halves.1.cell(), assigned_a.1.cell())?;
        }
        let a_pieces = self.decompose_abcd(region, row, a_value, postponed)?;
        Ok(RoundWordA::new(a_pieces, dense_halves, spread_halves))
    }

    pub(super) fn decompose_iv_e<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
        ...
        let row = get_decompose_e_row(RoundIdx::Init);
        // no need to look up the two pairs of (dense_half, spread_half) returned below,
        // since only the sum (spread_lo + spread_hi) will be used and the sum
        // is checked to be well formed in decompose_efgh.
        let (dense_halves, spread_halves) = self.assign_word_halves_fixed(region, row, e)?;
        let e_pieces = self.decompose_efgh(region, row, Value::known(e), postponed)?;
    ...
    pub(super) fn decompose_e<F: PrimeField>(
        ...
        region: &mut Region<'_, F>,
        round_idx: RoundIdx,
        assigned_e: &RoundWordDense<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<RoundWordE<F>, Error> {
        let e_value = assigned_e.value();

        let row = get_decompose_e_row(round_idx);
        // no need to look up the two pairs of (dense_half, spread_half) returned below,
        // since only the sum (spread_lo + spread_hi) will be used and the sum
        // is checked to be well formed in decompose_efgh.
        let (dense_halves, spread_halves) = self.assign_word_halves(region, row, e_value)?;
        let e_pieces = self.decompose_efgh(region, row, e_value, postponed)?;

        if let RoundIdx::Init = round_idx {
            region.constrain_equal(dense_halves.0.cell(), assigned_e.0.cell())?;
            region.constrain_equal(dense_halves.1.cell(), assigned_e.1.cell())?;
        }
        Ok(RoundWordE::new(e_pieces, dense_halves, spread_halves))
    }
    ...
    pub(super) fn assign_upper_sigma_1<F: PrimeField>(
        ...
        // we copy the advice from the previous state
        word.a_lo
            .spread
    ...
    pub(super) fn assign_ch<F: PrimeField>(
        ...
        self.assign_ch_outputs(region, row, p_0_even, p_0_odd, p_1_even, p_1_odd, postponed)
    }
    ...
    pub(super) fn assign_maj<F: PrimeField>(
        ...
        self.assign_maj_outputs(region, row, m_0_even, m_0_odd, m_1_even, m_1_odd, postponed)
    }
    ...
    pub(super) fn assign_h_prime<F: PrimeField>(
        ...
        round_idx: MainRoundIdx,
        h: RoundWordDense<F>,
        ch: (AssignedBits<16, F>, AssignedBits<16, F>),
        ch_neg: (AssignedBits<16, F>, AssignedBits<16, F>),
        sigma_1: (AssignedBits<16, F>, AssignedBits<16, F>),
        k: u32,
        w: &(AssignedBits<16, F>, AssignedBits<16, F>),
        ...
        {
            AssignedBits::<16, F>::assign_bits_fixed(region, || "k_lo", a_6, row - 1, k_lo)?;
            AssignedBits::<16, F>::assign_bits_fixed(region, || "k_hi", a_6, row, k_hi)?;
        }
        ...
        // Assign and copy ch_hi
        // No need to copy ch_lo, as the cell at a_1 of current row is included into the
        // gate
        ...
        // Assign h_prime_lo, h_prime_hi, h_prime_carry
        {
            ...
            let carry_lsb = h_prime_carry.map(|value| value & 1);
            let carry_msbits = h_prime_carry
                .zip(carry_lsb)
                .map(|(carry, lsb)| (carry - lsb) >> 1);

            region.assign_advice(
                || "h_prime carry_lsb",
                a_9,
                row - 1,
                || carry_lsb.map(F::from),
            )?;
            region.assign_advice(
                || "h_prime carry_msbits",
                a_9,
                row,
                || carry_msbits.map(F::from),
            )?;
            ...
            let h_prime_lo = AssignedBits::<16, F>::assign_bits(
                region,
                || "h_prime_lo",
                a_7,
                row + 1,
            ...
            let h_prime_hi = AssignedBits::<16, F>::assign_bits(
                region,
                || "h_prime_hi",
                a_8,
                row + 1,
            ...
            // push the low and high halves of h_prime for later range check 16-bit
            assigned_halves.push(h_prime_lo.clone());
            assigned_halves.push(h_prime_hi.clone());

            Ok((h_prime_lo, h_prime_hi).into())
        }
    }

    // takes two state variables to construct the new state, i.e. st_new = st_old +
    // {a,b,..,h}
    pub(super) fn assign_state_addition<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        row: usize,
        term1: RoundWordDense<F>,
        term2: RoundWordDense<F>,
    ) -> Result<RoundWordDense<F>, Error> {
        self.s_add_halves.enable(region, row)?;
        ...
        term1.0.copy_advice(|| "term1_lo", region, a_3, row)?;
        term1.1.copy_advice(|| "term1_hi", region, a_4, row)?;
        term2.0.copy_advice(|| "term2_lo", region, a_5, row)?;
        term2.1.copy_advice(|| "term2_hi", region, a_6, row)?;

        let (st_new, st_new_carry) = sum_with_carry(vec![
            (term1.0.value_u16(), term1.1.value_u16()),
            (term2.0.value_u16(), term2.1.value_u16()),
        ]);
        let st_new_dense = self.assign_word_halves_dense(region, row, a_7, row, a_8, st_new)?;
        region.assign_advice(|| "a_new_carry", a_9, row, || st_new_carry.map(F::from))?;
    ...
    pub(super) fn assign_e_new<F: PrimeField>(
        ...
        // push the low and high halves of e_new for later range check 16-bit
        assigned_halves.push(e_new_dense.0.clone());
        assigned_halves.push(e_new_dense.1.clone());
    ...
    pub(super) fn assign_a_new<F: PrimeField>(
        ...
        region: &mut Region<'_, F>,
        round_idx: MainRoundIdx,
        maj: (AssignedBits<16, F>, AssignedBits<16, F>),
        sigma_0: (AssignedBits<16, F>, AssignedBits<16, F>),
        h_prime: RoundWordDense<F>,
        assigned_halves: &mut Vec<AssignedBits<16, F>>,
    ) -> Result<RoundWordDense<F>, Error> {
        ...
        // push the low and high halves of a_new for later range check 16-bit
        assigned_halves.push(a_new_dense.0.clone());
        assigned_halves.push(a_new_dense.1.clone());
    ...
    pub fn assign_word_halves_dense<F: PrimeField>(
        ...
        let lo = {
            ...
            AssignedBits::<16, F>::assign_bits(region, || "lo", lo_col, lo_row, lo)?
        };

        let hi = {
            let hi: Value<[bool; 16]> = word.map(|w| w[16..].try_into().unwrap());
            AssignedBits::<16, F>::assign_bits(region, || "hi", hi_col, hi_row, hi)?
        };

        Ok((lo, hi).into())
    }

    // Same as `assign_word_halves_dense` but copies the advice from some fixed
    // values
    pub fn assign_word_halves_dense_fixed<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        lo_row: usize,
        lo_col: Column<Advice>,
        ...
        let word: [bool; 32] = i2lebsp(word.into());

        let lo = {
            let lo: [bool; 16] = word[..16].try_into().unwrap();
            AssignedBits::<16, F>::assign_bits_fixed(region, || "lo", lo_col, lo_row, lo)?
        };

        let hi = {
            let hi: [bool; 16] = word[16..].try_into().unwrap();
            AssignedBits::<16, F>::assign_bits_fixed(region, || "hi", hi_col, hi_row, hi)?
        };
    ...
    // Same as assign_word_halves_fixed but uses a fixed value to assign the words
    #[allow(clippy::type_complexity)]
    pub fn assign_word_halves_fixed<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        row: usize,
        word: u32,
        ...
        let w_lo =
            SpreadVar::without_lookup_fixed(region, a_7, row, a_8, row, SpreadWord::new(lo))?;
        let w_hi = SpreadVar::without_lookup_fixed(
            region,
            a_7,
            row + 1,
            a_8,
