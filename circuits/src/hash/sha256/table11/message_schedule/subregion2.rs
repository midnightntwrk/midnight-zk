...
use super::{
    super::{SpreadVar, SpreadWord, Table11Assignment},
    schedule_util::*,
    subregion1::AssignedHalves,
    MessageScheduleConfig, MessageWord,
};
use crate::hash::sha256::{
    table11::spread_table::PostponedSpreadVar,
...
#[derive(Clone, Debug)]
pub struct Subregion2Word<F: PrimeField> {
    index: usize,
    a: AssignedBits<3, F>,
    b: AssignedBits<4, F>,
    c: AssignedBits<3, F>,
    e: AssignedBits<1, F>,
    f: AssignedBits<1, F>,
    spread_d: AssignedBits<14, F>,
    spread_g: AssignedBits<26, F>,
}

impl<F: PrimeField> Subregion2Word<F> {
...
impl MessageScheduleConfig {
    // W_[14..49]
    pub fn assign_subregion2<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        lower_sigma_0_output: Vec<(AssignedBits<16, F>, AssignedBits<16, F>)>,
        w: &mut Vec<MessageWord<F>>,
        w_halves: &mut Vec<(AssignedBits<16, F>, AssignedBits<16, F>)>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<Vec<AssignedHalves<F>>, Error> {
        ...
        let mut lower_sigma_0_v2_results =
            Vec::<(AssignedBits<16, F>, AssignedBits<16, F>)>::with_capacity(SUBREGION_2_LEN);
        ...
        let mut new_word = |idx: usize,
                            sigma_0_output: &(AssignedBits<16, F>, AssignedBits<16, F>)|
         -> Result<Vec<AssignedHalves<F>>, Error> {
            // Decompose word into (3, 4, 3, 7, 1, 1, 13)-bit chunks
            let word = self.decompose_word(region, w[idx].value(), idx, postponed)?;

            // sigma_0 v2 and sigma_1 v2 on word
            lower_sigma_0_v2_results.push(self.lower_sigma_0_v2(
                region,
                word.clone(),
                postponed,
            )?);
            lower_sigma_1_v2_results.push(self.lower_sigma_1_v2(region, word, postponed)?);
            ...
            // Assign W_i, carry_i
            let temp_word = region.assign_advice(
            ...
            region.constrain_equal(temp_word.cell(), word.cell())?;
        ...
        let mut tmp_lower_sigma_0_v2_results: Vec<(AssignedBits<16, F>, AssignedBits<16, F>)> =
    ...
    fn decompose_word<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        word: Value<&Bits<32>>,
        index: usize,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<Subregion2Word<F>, Error> {
        ...
        let a = AssignedBits::<3, F>::assign_bits(region, || "a", a_3, row - 1, pieces[0].clone())?;

        // Assign `b` (4-bit piece) lookup
        let spread_b: Value<SpreadWord<4, 8>> = pieces[1].clone().map(SpreadWord::try_new);
        let spread_b = SpreadVar::with_lookup(region, &self.lookup, row + 1, spread_b, postponed)?;

        // Assign `c` (3-bit piece)
        let c = AssignedBits::<3, F>::assign_bits(region, || "c", a_4, row - 1, pieces[2].clone())?;

        // Assign `d` (7-bit piece) lookup
        let spread_d: Value<SpreadWord<7, 14>> = pieces[3].clone().map(SpreadWord::try_new);
        let spread_d = SpreadVar::with_lookup(region, &self.lookup, row, spread_d, postponed)?;

        // Assign `e` (1-bit piece)
        let e = AssignedBits::<1, F>::assign_bits(region, || "e", a_3, row + 1, pieces[4].clone())?;

        // Assign `f` (1-bit piece)
        let f = AssignedBits::<1, F>::assign_bits(region, || "f", a_4, row + 1, pieces[5].clone())?;

        // Assign `g` (13-bit piece) lookup
        let spread_g = pieces[6].clone().map(SpreadWord::<13, 26>::try_new);
        let spread_g = SpreadVar::with_lookup(region, &self.lookup, row - 1, spread_g, postponed)?;
    ...
    fn assign_lower_sigma_v2_pieces<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        row: usize,
        word: &Subregion2Word<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<(), Error> {
        let a_3 = self.extras[0];
        let a_4 = self.extras[1];
        ...
        // Witness `spread_a`
        let assigned_spread_a = AssignedBits::<6, F>::assign_bits(
            region,
            || "spread_a",
            a_4,
            row + 1,
        ...
        {
            AssignedBits::<2, F>::assign_bits(region, || "b_lo", a_3, row - 1, b_lo)?;

            AssignedBits::<4, F>::assign_bits(region, || "spread_b_lo", a_4, row - 1, spread_b_lo)?;
        };
        ...
        {
            AssignedBits::<2, F>::assign_bits(region, || "b_hi", a_5, row - 1, b_hi)?;

            AssignedBits::<4, F>::assign_bits(region, || "spread_b_hi", a_6, row - 1, spread_b_hi)?;
        };
        ...
        // Witness `spread_c`
        let assigned_spread_c = AssignedBits::<6, F>::assign_bits(
            region,
            || "spread_c",
            a_6,
            row + 1,
    ...
    fn lower_sigma_0_v2<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        word: Subregion2Word<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
        ...
        self.assign_lower_sigma_v2_pieces(region, row, &word, postponed)?;
    ...
    fn lower_sigma_1_v2<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        word: Subregion2Word<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
