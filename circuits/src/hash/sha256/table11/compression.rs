...
use super::{
    super::DIGEST_SIZE, decomposition::SpreadTableConfig, Range16Config, SpreadInputs, SpreadVar,
    Table11Assignment,
};
...
impl<F: PrimeField> Table11Assignment<F> for CompressionConfig {}
...
impl CompressionConfig {
    ...
    pub(super) fn initialize_with_iv<F: PrimeField>(
        ...
        let postponed = layouter.assign_region(
            ...
            |mut region| {
                ...
                new_state = self.initialize_iv(&mut region, &mut postponed)?;
                Ok(postponed)
            },
        )?;

        layouter.assign_region(
            || "postponed from initialize_with_iv",
    ...
    pub(super) fn initialize_with_state<F: PrimeField>(
        ...
        let postponed = layouter.assign_region(
            ...
            |mut region| {
                let mut postponed = Vec::new();
                new_state =
                    self.initialize_state(&mut region, init_state.clone(), &mut postponed)?;
                Ok(postponed)
            },
        )?;

        layouter.assign_region(
            || "postponed from initialize_with_state",
            |mut region| decomposition_config.assign_postponed_vector(&mut region, 0, &postponed),
        )?;
    ...
    pub(super) fn compress<F: PrimeField>(
        ...
        let (postponed, assigned_halves) = layouter.assign_region(
            || "compress",
            |mut region| {
                let mut postponed = Vec::new();
                ...
                Ok((postponed, assigned_halves))
            },
        )?;

        layouter.assign_region(
            || "postponed from compress",
            |mut region| decomposition_config.assign_postponed_vector(&mut region, 0, &postponed),
        )?;

        layouter.assign_region(
            || "range check the assigned halves from compress",
            |mut region| range16.assign_halves_vector(&mut region, 0, &assigned_halves),
        )?;
...
mod tests {
    ...
    use crate::{
        hash::sha256::{table11::decomposition::SpreadTableConfig, BlockWord},
    ...
    fn compress() {
        ...
        impl<F: PrimeField> Circuit<F> for MyCircuit {
            ...
            fn synthesize(
                ...
                let decomposition_config = config.decomposition_config.clone();
                ...
                let initial_state =
                    compression.initialize_with_iv(&mut layouter, decomposition_config.clone())?;
