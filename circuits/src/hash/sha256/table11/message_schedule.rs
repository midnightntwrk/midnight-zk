...
use super::{super::BLOCK_SIZE, decomposition::SpreadTableConfig, SpreadInputs, Table11Assignment};
...
use crate::hash::sha256::{AssignedBits, BlockWord, ROUNDS};

#[derive(Clone, Debug)]
pub(crate) struct MessageWord<F: PrimeField>(AssignedBits<32, F>);
...
impl<F: PrimeField> Table11Assignment<F> for MessageScheduleConfig {}

impl MessageScheduleConfig {
    /// Configures the message schedule.
    ///
    /// `message_schedule` is the column into which the message schedule will be
    /// placed. The caller must create appropriate permutations in order to
    /// load schedule words into the compression rounds.
    ///
    /// `extras` contains columns that the message schedule will only use for
    /// internal gates, and will not place any constraints on (such as
    /// lookup constraints) outside itself.
    ...
    pub(super) fn configure<F: PrimeField>(
        ...
        meta.create_gate("s_decompose_1", |meta| {
            ...
            ScheduleGate::s_decompose_1(s_decompose_1, a, b, c, d, word)
        });
        ...
        meta.create_gate("s_decompose_2", |meta| {
            ...
            ScheduleGate::s_decompose_2(s_decompose_2, a, b, c, d, e, f, g, word)
        });
        ...
        meta.create_gate("s_decompose_3", |meta| {
            ...
            ScheduleGate::s_decompose_3(s_decompose_3, a, b, c, d, word)
        });
    ...
    > {
        ...
        let postponed = layouter.assign_region(
            ...
            |mut region| {
                ...
                let lower_sigma_0_output =
                    self.assign_subregion1(&mut region, &input[1..14], &mut postponed)?;
        ...
        layouter.assign_region(
            || "postponed from process message",
            |mut region| decomposition_config.assign_postponed_vector(&mut region, 0, &postponed),
        )?;
...
mod tests {
    ...
    use crate::{
        hash::sha256::{table11::decomposition::SpreadTableConfig, util::lebs2ip, BlockWord},
    ...
    fn message_schedule() {
        ...
        impl<F: PrimeField> Circuit<F> for MyCircuit {
            ...
            fn synthesize(
                ...
                SpreadTableConfig::load(config.decomposition_config.clone(), &mut layouter)?;
                // Provide input
                // Test vector: "abc"
                let inputs: [BlockWord; BLOCK_SIZE] = msg_schedule_test_input();

                let (w, _) = config.message_schedule.process(
                    &mut layouter,
                    inputs,
