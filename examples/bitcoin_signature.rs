...
struct BitcoinSigExample {
    // Public inputs:
    pk: Value<PK>,
    msg_bytes: Value<Message>,
...
impl Relation for BitcoinSigExample {
    ...
    fn from_statement(instance: &Self::Instance, witness: &Self::Witness) -> Self {
        BitcoinSigExample {
            pk: Value::known(instance.0),
            msg_bytes: Value::known(instance.1),
            signature: Value::known(*witness),
        }
    }
    ...
    fn circuit(&self, std_lib: &ZkStdLib, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        ...
        let msg_bytes = std_lib.assign_many(layouter, &self.msg_bytes.transpose_array())?;
        msg_bytes
            .iter()
            .try_for_each(|byte| std_lib.constrain_as_public_input(layouter, byte))?;

        // Assign the signature as a witness.
        let rx = secp256k1_base.assign(layouter, self.signature.map(|(rx, _)| rx))?;
        let s = secp256k1_scalar.assign(layouter, self.signature.map(|(_, s)| s))?;
...
fn main() {
    ...
    let srs = filecoin_srs(BitcoinSigExample::K);

    let vk = compact_std_lib::setup_vk::<BitcoinSigExample>(&srs);

    if cfg!(feature = "check_vk") {
        check_vk::<G1Affine, MidnightCircuit<BitcoinSigExample>>(&vk);
        return;
    }

    let pk = compact_std_lib::setup_pk::<BitcoinSigExample>(&srs, &vk);
    ...
    let proof = compact_std_lib::prove::<BitcoinSigExample>(&srs, &pk, &instance, &witness);

    compact_std_lib::verify::<BitcoinSigExample>(&srs, &vk, &instance, proof)
}
