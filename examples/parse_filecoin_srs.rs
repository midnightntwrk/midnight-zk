...
fn read_points() -> std::io::Result<(Vec<WrapperPoint<G1Affine>>, Vec<WrapperPoint<G2Affine>>)> {
    let mut fd = OpenOptions::new()
        .read(true)
        .open("./examples/assets/phase1radix2m19")?;
...
fn main() -> std::io::Result<()> {
    if OpenOptions::new()
        .read(true)
        .open("./examples/assets/bls_filecoin_2p19")
        .is_ok()
    {
        return Ok(());
    }
    ...
    let params = ParamsKZG::<Bls12>::new(K as u32);
    ...
    let mut file =
        File::create("./examples/assets/bls_filecoin_2p19").expect("Failed to create file");
