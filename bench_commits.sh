...
# Branch 1 (current): bench each PR-merge that lands a perf-relevant change.
# The first entry is the baseline — it adds the Criterion infrastructure on
# top of main with no perf-relevant change. The CI-only commit 02e28d19 is
# skipped as it does not affect prove time.
COMMITS=(
    a01ba2be  # baseline — bench infrastructure on top of main.
    f4b28fd1  # PR #350: affine SRS.
    0efefad5  # PR #352: FFT (cached twiddles + pruned DIF).
    796e848b  # PR #351: Graph Evaluator (flatten + batch).
    e81c7fc4  # PR #353: parallelization (logup, perm, SHPLONK).
)
