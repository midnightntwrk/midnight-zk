...
/// value that each limb should take for the emulated field element to be
...
pub fn well_formedness_bounds<F, K, P>() -> Vec<u32>
where
    F: PrimeField,
    K: PrimeField,
    P: FieldEmulationParams<F, K>,
    ...
    P: FieldEmulationParams<F, K>,
{
    ...
    /// limb_bounds match the range [0, base).
    /// AssignedField whose range is more restricted but included in [0, base)
    /// are also considered well-formed.
    pub fn is_well_formed(&self) -> bool {
        self.limb_bounds.iter().fold(true, |acc, (lower, upper)| {
            debug_assert!(lower <= upper);
            acc && !BI::is_negative(lower) && upper.bits() <= P::LOG2_BASE as u64
        })
    }
    ...
    N: NativeInstructions<F>,
{
    fn assign(
        ...
        let x_cells = (0..P::NB_LIMBS)
            ...
            .zip(well_formedness_bounds::<F, K, P>().iter())
    ...
    N: NativeInstructions<F>,
{
    ...
    fn add_constant(
        ...
        let z_limb_values = x
            .limb_values
            .iter()
            .zip(k_limbs.iter().map(|ki| bigint_to_fe::<F>(ki)))
            .map(|(xi, ki)| self.native_gadget.add_constant(layouter, xi, ki))
            .collect::<Result<Vec<_>, _>>()?;
...
mod tests {
    ...
    use crate::{
        ...
        instructions::{arithmetic, assertions, control_flow, decomposition, equality, zero},
    };
