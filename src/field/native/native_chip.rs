...
use halo2_proofs::{
    ...
    plonk::{
        Advice, Column, ConstraintSystem, Constraints, ErrorFront as Error, Fixed, Instance,
        Selector,
    },
    poly::Rotation,
};
...
pub(crate) const NB_ARITH_COLS: usize = 5;
...
impl<F: PrimeField> NativeChip<F> {
    ...
    /// current offset. This function does not assign values, it assumes
    /// they have already been assigned.
    /// This function increases the offset by one at the end only if
    /// `q_next_coeff != 0`.
    fn custom(
        &self,
        ...
        assert!(coeffs.len() <= NB_ARITH_COLS);
        coeffs.resize(NB_ARITH_COLS, F::ZERO);
