...
where
    ...
    NativeGadget<F, CoreDecomposition, NativeArith>:
        CanonicityInstructions<F, AssignedNative<F>> + AssertionInstructions<F, AssignedBit<F>>,
{
...
// Inherit Bit Equality Instructions.
...
impl<F: PrimeField> FromScratch<F> for NativeGadget<F, P2RDecompositionChip<F>, NativeChip<F>> {
    ...
    fn configure_from_scratch(
        ...
        // We share the colums with the pow2range chip. This requires to share as many
        // available advice columns from the native chip as possible *except*
        // the first one
