...
/// Decompose the given field element into little-endian bits, with trailing
/// zeros if necessary, to guarantee that the output has length `F::NUM_BITS`.
pub fn fe_to_le_bits<F: PrimeField>(value: &F) -> Vec<bool> {
    let big = fe_to_big(*value);
    let mut bits: Vec<bool> = (0..big.bits()).map(|i| big.bit(i)).collect();
    bits.resize(F::NUM_BITS as usize, false);
    bits
}
