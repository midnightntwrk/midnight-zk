...
pub struct Subregion3Word<F: PrimeField> {
    ...
    b: AssignedBits<7, F>,
    c: AssignedBits<2, F>,
    spread_a: AssignedBits<20, F>,
    spread_d: AssignedBits<26, F>,
}

impl<F: PrimeField> Subregion3Word<F> {
...
impl MessageScheduleConfig {
    // W_[49..62]
    pub fn assign_subregion3<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        lower_sigma_0_v2_output: Vec<(AssignedBits<16, F>, AssignedBits<16, F>)>,
        ...
        let mut new_word = |idx: usize| -> Result<(), Error> {
            ...
            let subregion3_word =
                self.decompose_subregion3_word(region, w[idx].value(), idx, postponed)?;

            // sigma_1 on subregion3_word
            let (r_0_even, r_1_even) = self.lower_sigma_1(region, subregion3_word, postponed)?;
    ...
    fn decompose_subregion3_word<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        word: Value<&Bits<32>>,
        index: usize,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
    ) -> Result<Subregion3Word<F>, Error> {
        ...
        let spread_a = pieces[0].clone().map(SpreadWord::<10, 20>::try_new);
        let spread_a = SpreadVar::with_lookup(region, &self.lookup, row + 1, spread_a, postponed)?;

        // Assign `b` (7-bit piece)
        let b = AssignedBits::<7, F>::assign_bits(region, || "b", a_4, row + 1, pieces[1].clone())?;

        // Assign `c` (2-bit piece)
        let c = AssignedBits::<2, F>::assign_bits(region, || "c", a_3, row + 1, pieces[2].clone())?;

        // Assign `d` (13-bit piece) lookup
        let spread_d = pieces[3].clone().map(SpreadWord::<13, 26>::try_new);
    ...
    fn lower_sigma_1<F: PrimeField>(
        &self,
        region: &mut Region<'_, F>,
        word: Subregion3Word<F>,
        postponed: &mut Vec<Box<dyn PostponedSpreadVar>>,
        ...
        // b_hi (3-bit chunk)
        {
            ...
            // postpone the check of b_hi
            postponed.push(Box::new(assigned_b_hi.clone()));
        }
        ...
        // Witness `spread_c`
        {
            ...
            AssignedBits::<4, F>::assign_bits(region, || "spread_c", a_4, row + 1, spread_c)?;
        }
