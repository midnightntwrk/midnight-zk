...
use halo2_proofs::{
    ...
    plonk::{
        Advice, Any, Column, ConstraintSystem, Constraints, ErrorFront as Error, Expression, Fixed,
        Selector,
    },
    poly::Rotation,
};
...
use crate::{
    hash::poseidon::{
        cpu::{Absorbing, Domain, Mds, Spec, Squeezing, State},
...
///
/// The chip is implemented using a single round per row for full rounds, and
/// two rounds per row for partial rounds.
#[derive(Clone, Debug)]
...
impl<F: Field, const WIDTH: usize, const RATE: usize> Pow5Chip<F, WIDTH, RATE> {
    ...
    pub fn configure<S: Spec<F, WIDTH, RATE>>(
        ...
        // This allows state words to be initialized (by constraining them equal to
        // fixed values), and used in a permutation from an arbitrary region.
        // rc_a is used in every permutation round, while rc_b is empty in the
        // initial and final full rounds, so we use rc_b as "scratch space" for
        // fixed values (enabling potential layouter optimisations).
...
/// A [`Pow5Chip`] with an additional [`Domain`], in order to implement
/// [`PoseidonSpongeInstructions`].
#[derive(Clone, Debug)]
pub struct Pow5SpongeChip<F: Field, D: Domain<F, RATE>, const WIDTH: usize, const RATE: usize> {
    /// The underlying [`Pow5Chip`].
    chip: Pow5Chip<F, WIDTH, RATE>,
    /// The domain for this Poseidon sponge.
    domain: D,
}

impl<F: Field, D: Domain<F, RATE>, const WIDTH: usize, const RATE: usize>
    Pow5SpongeChip<F, D, WIDTH, RATE>
{
    /// Configures this chip for use in a circuit by calling the `config` method
    /// on [`Pow5Chip`].
    ///
    /// # Side-effects
    ///
    ...
    /// Construct a [`Pow5SpongeChip`] from a [`Pow5Config`] and a [`Domain`].
    pub fn construct(config: Pow5Config<F, WIDTH, RATE>, domain: D) -> Self {
        let chip = Pow5Chip::construct(config);
        Pow5SpongeChip { chip, domain }
    }

    /// Construct a [`Pow5SpongeChip`] from a [`Pow5Chip`] and a [`Domain`].
    pub fn from_pow5_chip(chip: Pow5Chip<F, WIDTH, RATE>, domain: D) -> Self {
        Pow5SpongeChip { chip, domain }
    }
}
...
impl<
        ...
        const WIDTH: usize,
        const RATE: usize,
    > PoseidonInstructions<F, S, WIDTH, RATE> for Pow5SpongeChip<F, D, WIDTH, RATE>
{
    type Word = <Pow5Chip<F, WIDTH, RATE> as PoseidonInstructions<F, S, WIDTH, RATE>>::Word;
    ...
    fn permute(
        ...
        <Pow5Chip<F, WIDTH, RATE> as PoseidonInstructions<F, S, WIDTH, RATE>>::permute(
            &self.chip,
            layouter,
            initial_state,
        )
...
impl<F: Field, D: Domain<F, RATE>, const WIDTH: usize, const RATE: usize> Chip<F>
    for Pow5SpongeChip<F, D, WIDTH, RATE>
{
    type Config = <Pow5Chip<F, WIDTH, RATE> as Chip<F>>::Config;

    type Loaded = <Pow5Chip<F, WIDTH, RATE> as Chip<F>>::Loaded;
...
impl<
        ...
        const WIDTH: usize,
        const RATE: usize,
    > PoseidonSpongeInstructions<F, S, D, WIDTH, RATE> for Pow5SpongeChip<F, D, WIDTH, RATE>
{
    fn initial_state(
        &self,
        layouter: &mut impl Layouter<F>,
        ...
        let state = layouter.assign_region(
            || format!("initial state for domain {}", self.domain.name()),
            |mut region| {
                ...
                load_state_word(RATE, self.domain.initial_capacity_element())?;
    ...
    fn add_input(
        ...
        let config = self.config();
        layouter.assign_region(
            || format!("add input for domain {}", self.domain.name()),
            |mut region| {
...
impl<F: Field> InnerValue for StateWord<F> {
    type Element = F;
...
mod tests {
    ...
    use crate::hash::poseidon::{cpu::Spec, permute};
        ...
        for PermuteCircuit<S, WIDTH, RATE>
    {
        ...
        fn synthesize(
            ...
            permute::<_, S, WIDTH, RATE>(&mut expected_final_state, &mds, &round_constants);
