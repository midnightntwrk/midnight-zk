...
use blstrs::{AffinePoint, ExtendedPoint, Scalar as BlsScalar, SubgroupPoint};
use ff::PrimeField;
...
/// field.
pub trait CircuitCurve: Curve<Scalar = <Self::CryptographicGroup as Group>::Scalar> {
    /// Base field over which the EC is defined.
    type Base: PrimeField;
    /// Cryptographic group
    type CryptographicGroup: Group + Into<Self>;
...
impl CircuitCurve for ExtendedPoint {
    type Base = BlsScalar;
    type CryptographicGroup = SubgroupPoint;
    ...
    fn into_subgroup(self) -> Self::CryptographicGroup {
        <ExtendedPoint as CofactorGroup>::into_subgroup(self)
            .expect("Point should be part of the subgroup")
    }
}

impl EdwardsCurve for ExtendedPoint {
