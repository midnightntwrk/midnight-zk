...
use crate::{
    ...
    instructions::{
        AssignmentInstructions, BinaryInstructions, DecompositionInstructions, EccInstructions,
        EqualityInstructions, FieldInstructions,
    },
    ...
    C: EdwardsCurve + MapToCurveCPU<C> + MapToEdwardsParams<C::Base>,
{
    fn map_to_curve(
        ...
        if C::COFACTOR == 1u128 {
            ...
            // Map into subgroup
            let cofactor = self.assign_fixed(layouter, C::Scalar::from_u128(C::COFACTOR))?;
            self.mul(layouter, &cofactor, &point)
        }
    }
}
