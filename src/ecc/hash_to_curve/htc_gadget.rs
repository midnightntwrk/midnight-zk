...
where
    ...
    E: EccInstructions<F, C> + MapToCurveInstructions<F, C>,
{
    fn hash_to_curve(input: &I::Element) -> C::CryptographicGroup {
        let mut state = <H as SpongeCPU<I::Element, C::Base>>::init();
        <H as SpongeCPU<I::Element, C::Base>>::absorb(&mut state, &[input.clone()]);
...
where
    ...
    E: EccInstructions<F, C> + MapToCurveInstructions<F, C>,
{
    fn hash_to_curve(&self, layouter: &mut impl Layouter<F>, input: &I) -> Result<E::Point, Error> {
        let mut state = self.sponge_chip.init(layouter)?;
        self.sponge_chip
            .absorb(layouter, &mut state, &[input.clone()])?;
