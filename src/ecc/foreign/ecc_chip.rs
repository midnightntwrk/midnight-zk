...
use crate::{
    ...
    utils::util::{bigint_to_fe, fe_to_big, fe_to_le_bits, glv_scalar_decomposition},
};
    ...
    B: FieldEmulationParams<F, C::Base>,
{
    fn as_public_input(p: &C::CryptographicGroup) -> Vec<F> {
        ...
        let (x, y) = (*p).into().coordinates().expect("Valid affine point.");
        [
    ...
    N: NativeInstructions<F>,
{
    ...
    fn assert_not_equal_to_fixed(
        ...
        if constant.is_identity().into() {
            ...
            let q = self.assign_fixed(layouter, constant)?;
            self.assert_not_equal(layouter, p, &q)
        }
    }
}
    ...
    N: NativeInstructions<F>,
{
    ...
    fn mul_by_constant(
        ...
        let scalar_bits = fe_to_le_bits(&scalar)
            .iter()
    ...
    N: NativeInstructions<F>,
{
    ...
    fn assert_add(
        ...
        // satisfies all the preconditions of [assert_slope].
    ...
    fn incomplete_assert_different_x(
        ...
        // The difference between the two different representations (of the same
        // integer) limb-wise is always the limb representation of the modulus
        // associated to them. We will assert that the most significant limbs of
        // `p.x` and `q.x` differ by an amount which is not in {0, k, -k}, where
        // `k` is the most significant limb of the modulus in limbs form.
        // This guarantees that `p.x` and `q.x` are different emulated field elements.
        // This is sound, as the assertion will be violated when given equal emulated
        // field elements, even if they use different limb representation.
        // On the other hand, this check is not complete, as e.g. there could exist
        // different emulated field elements with the same most significant limb.
        let base = BigInt::from(2).pow(B::LOG2_BASE);
        let modulus_limbs = bi_to_limbs(B::NB_LIMBS, &base, &p.x.modulus());
        let px_limbs = p.x.limb_values();
        let qx_limbs = q.x.limb_values();
        let msl_modulus = bigint_to_fe(modulus_limbs.last().unwrap());
        let msl_px = px_limbs.last().unwrap();
        let msl_qx = qx_limbs.last().unwrap();

        // We assert that `(msl_px - msl_qx) not in {0, msl_modulus, -msl_modulus}`.
        // TODO: the following could be done more efficiently if we had dedicated
        // instructions in the native gadget.
        let diff = self.native_gadget.sub(layouter, msl_px, msl_qx)?;
        self.native_gadget.assert_non_zero(layouter, &diff)?;
        self.native_gadget
            .assert_not_equal_to_fixed(layouter, &diff, msl_modulus)?;
        self.native_gadget
            .assert_not_equal_to_fixed(layouter, &diff, msl_modulus.neg())
    }
    ...
    fn windowed_msm<const WS: usize>(
        ...
        for s_bits in scalars.iter() {
            ...
            // Reverse to get big-endian order, for the double-and-and.
        ...
        for (i, p) in bases.iter().enumerate() {
            ...
            for _ in 1..(1usize << WS) {
                ...
                // (2) holds because all the bases were assert to not be the identity.
                // (3) is asserted here, this assertion will not hinder completeness except
                //     with negligible probability.
                self.incomplete_assert_different_x(layouter, &acc, p)?;
                acc = self.incomplete_add(layouter, &acc, p)?;
                // assert that acc.x and acc.y have well-formed bounds.
...
mod tests {
    ...
    use crate::{
        ...
        instructions::{assertions, control_flow, ecc, equality, zero},
    };
