    ...
    C: CircuitCurve,
{
    ...
    fn hash_to_curve(input: &Input) -> C::CryptographicGroup;
}
...
pub mod tests {
        ...
        HashToCurveChip: HashToCurveInstructions<F, C, I, EccChip> + FromScratch<F>,
    {
        ...
        if cost_model {
            circuit_to_json(log2_nb_rows, chip_name, "hash_to_curve", &[&[]], circuit);
        }
    }
    ...
    where
        ...
        HashToCurveChip: HashToCurveInstructions<F, C, I, EccChip> + FromScratch<F>,
    {
        [I::inner_zero(), I::inner_one(), I::sample_inner(OsRng)]
            .iter()
            .for_each(|input| {
                let expected =
                    <HashToCurveChip as HashToCurveCPU<C, I::Element>>::hash_to_curve(input);
