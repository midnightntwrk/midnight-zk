...
pub trait SpongeCPU<Input, Output> {
    ...
    fn absorb(state: &mut Self::StateCPU, input: &[Input]);
...
pub mod tests {
            ...
            AssignmentInstructions<F, Input> + AssertionInstructions<F, Output> + FromScratch<F>,
    {
        ...
        fn synthesize(
            ...
            let vec_to_values = |vec: &Vec<Input::Element>| -> Vec<Value<_>> {
                vec.iter()
            ...
            let mut input_idx = 0;
            let mut state = chip.init(&mut layouter)?;
            let mut cpu_state = <SpongeChip as SpongeCPU<Input::Element, Output::Element>>::init();

            for step in self.sequence.iter() {
                for _nr_absorb in 0..step.0 {
                    let input = assign_chip
                        .assign_many(&mut layouter, &vec_to_values(&self.inputs[input_idx]))?;

                    chip.absorb(&mut layouter, &mut state, &input)?;
            ...
            AssignmentInstructions<F, Input> + AssertionInstructions<F, Output> + FromScratch<F>,
    {
        ...
        if cost_model {
            circuit_to_json(k, chip_name, "sponge", &[&[]], circuit);
        }
    }
}
