...
macro_rules! plonk_api {
    ($name:ident, $engine:ty, $native:ty, $projective:ty) => {
            ...
            Relation: Circuit<$native>,
        {
            ...
            pub fn setup_vk(
                ...
                circuit: &Relation,
            ) -> VerifyingKey<$native, FflonkScheme<$engine>> {
                #[cfg(test)]
            ...
            pub fn setup_pk(
                circuit: &Relation,
                vk: &VerifyingKey<$native, FflonkScheme<$engine>>,
            ) -> ProvingKey<$native, FflonkScheme<$engine>> {
                #[cfg(test)]
                let start = Instant::now();
            ...
            pub fn prove<H>(
                params: &ParamsKZG<$engine>,
                pk: &ProvingKey<$native, FflonkScheme<$engine>>,
                circuit: &Relation,
                ...
                $native: Hashable<H> + Sampleable<H>,
            {
                ...
                let proof = {
                    ...
                    create_proof::<$native, FflonkScheme<$engine>, CircuitTranscript<H>, Relation>(
                        params,
                        pk,
                        circuit,
            ...
            pub fn verify<H>(
                ...
                vk: &VerifyingKey<$native, FflonkScheme<$engine>>,
                instance_commitments: &[FflonkCommitment<$engine>],
                pi: &[&[$native]],
                proof: &[u8],
            ) -> Result<(), Error>
            where
                ...
                $native: Hashable<H> + Sampleable<H>,
            {
                ...
                #[cfg(test)]
                let start = Instant::now();
                let res = prepare::<$native, FflonkScheme<$engine>, CircuitTranscript<H>>(
                    vk,
...
pub fn load_srs(source: SrsSource, k: u32, cs_degree: usize) -> ParamsKZG<Bls12> {
    // Fail-fast on the field's 2-adicity limit.
    assert!(
        k <= Fq::S,
        "load_srs: requested k = {k} exceeds the 2-adicity of the field = {}",
        Fq::S
    );
