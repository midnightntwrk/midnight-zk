...
//!
//! Loads test credentials from `examples/identity/passport/credentials/`,
...
const CRED_DIR: &str = concat!(
    ...
    "/examples/identity/passport/credentials"
);
...
fn run() {
    ...
    // Default to credential_2 (DOB 1991-12-14, passes 18+ age check).
    let cred_name = std::env::args().nth(1).unwrap_or_else(|| "credential_2".to_string());
    ...
    // Reference date for age verification (YYYYMMDD encoding).
    // Public inputs for age verification.
    let reference_date = 20260329u64;
    let century_base = 26u64; // YY window: [1926, 2026)
    println!("  Reference date: {reference_date} (YYYYMMDD)");
    println!("  Century base:   {century_base} (year window [{}, {}))\n",
        1900 + century_base, 2000 + century_base);
    let instance = passport::circuit::Instance {
        csca_merkle_root: witness.csca_merkle_root(),
        reference_date: F::from(reference_date),
        century_base: F::from(century_base),
    };
