...
fn msm_blst(c: &mut Criterion) {
    ...
    let (bases, coeffs) = setup::<xxxx_curves::G1Affine>();
    ...
    for (b_index, b) in BITS.iter().enumerate() {
        for k in MULTICORE_RANGE {
            ...
            let points: Vec<xxxx_curves::G1Projective> = bases.iter().map(Into::into).collect();
            group.bench_function(BenchmarkId::new("Blst", id), |b| {
                b.iter(|| xxxx_curves::G1Projective::multi_exp(&points[..n], &coeffs[b_index][..n]))
            });
        }
    }
