...
mod test {
    ...
    fn test_booth_encoding() {
        fn mul(scalar: &Fq, point: &G1Affine, window: usize) -> G1Affine {
            let u = scalar.to_repr();
            let n = Fq::NUM_BITS as usize / window + 1;

            let table = (0..=1 << (window - 1))
                .map(|i| point * Fq::from(i as u64))
                .collect::<Vec<_>>();
        ...
        let (scalars, points): (Vec<_>, Vec<_>) = (0..10)
            .map(|_| {
                ...
                let point = G1Projective::random(OsRng).to_affine();
                (scalar, point)
            })
            .unzip();
