...
use crate::bls12_381::{
    Fp, Fp12, Fp2, Fp6, Fq, G1Affine, G1Projective, G2Affine, G2Projective, Gt, MillerLoopResult,
};
...
impl<'de> Deserialize<'de> for Fq {
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        match Fq::from_u64s_le(&deser).into() {
...
mod tests {
    ...
    fn serde_scalar() {
        ...
        test_roundtrip(&f);
        assert_eq!(
            serde_json::from_slice::<Fq>(&hex::decode("5b312c302c302c305d").unwrap()).unwrap(),
            f
        );
    }
