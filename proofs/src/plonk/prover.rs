...
///
/// **Default behaviour** (`whole-h-poly` feature *disabled*): `h(X)` is split
/// into `quotient_poly_degree` limbs of degree `n-2` each, so that each can be
/// committed with an SRS of size `n` after 1 term for blinding. Each limb is
/// independently blinded and committed, and all `quotient_poly_degree`
...
///
/// **Alternative behaviour** (`whole-h-poly` feature *enabled*): `h(X)` is
/// committed as a single polynomial without splitting. A single commitment is
/// written to the transcript. The returned `Vec` contains exactly one element:
/// the full `h(X)` in coefficient form. In this mode the `params` **must**
/// supply an SRS of at least `(n-1) * quotient_poly_degree` elements (i.e.,
/// params generated with `k' >= log2(n * (d-1))`).
    ...
    CS::Commitment: Hashable<T::Hash>,
{
    ...
    // When the whole-h-poly feature is enabled, commit to h(X) in one go.
    ...
    #[cfg(feature = "whole-h-poly")]
    {
        ...
        let h_poly = Polynomial { values: h_poly, _marker: std::marker::PhantomData };
        let h_com = CS::commit(params, &h_poly);
        transcript.write(&h_com)?;
        return Ok(vec![h_poly]);
    }
