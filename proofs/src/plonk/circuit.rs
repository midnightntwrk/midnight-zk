...
impl<F: Field> Gate<F> {
    ...
    /// If gate selector is multipl. and simple, returns index of
    /// fixed column corresp. to this selector (after its conversion to a fixed
    /// column). Otherwise, returns `None`.
...
pub struct ConstraintSystem<F: Field> {
    ...
    ///   multipl., and `0` otherwise.
    /// * AFTER: Contains the fixed column indices of all simple, multipl.
    ///   selectors.
...
impl<'com, F: Field> ConstraintSystem<F> {
    ...
    pub fn directly_convert_selectors_to_fixed(
        ...
        // Adjust indices of simple, multipl. selectors: after converting
        // selectors to fixed columns, the simple selector index of a gate should
        // now track the index of the corresp. fixed column
