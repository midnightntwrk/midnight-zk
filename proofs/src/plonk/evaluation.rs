...
impl<F: WithSmallOrderMulGroup<3>> Evaluator<F> {
    ...
    pub fn new(cs: &ConstraintSystem<F>) -> Self {
        ...
        for lookup in cs.lookups.iter().flat_map(|l| l.split(cs.degree())) {
            ...
            // Now we compute the product of the lookup columns, and sum of the partial product of lookup
            // columns.
            ...
            for (n, lookup) in lookups.iter().enumerate() {
                ...
                parallelize(&mut values, |values, start| {
                    ...
                    for (i, value) in values.iter_mut().enumerate() {
                        ...
                        // Check the helper polynomial identity: h(X) * product = sum_partial_products.
