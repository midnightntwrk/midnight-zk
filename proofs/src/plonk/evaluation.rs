...
impl<F: WithSmallOrderMulGroup<3>> Evaluator<F> {
    ...
    pub fn new(cs: &ConstraintSystem<F>) -> Self {
        ...
        // Lookups
        for lookup in cs.lookups.iter().flat_map(logup::BatchedArgument::split) {
            let mut graph = GraphEvaluator::default();

            // Each input expression gets compressed with theta and added with beta
            ...
            // * product is at -2
            // * table is at -1
            // We are sure that the calculation is added (and did not exist previously).
            // Nonetheless, we perform a runtime check:
    ...
    pub(crate) fn evaluate_h<B: PolynomialRepresentation>(
            ...
            .zip(permutations.iter())
        {
            ...
            for (n, lookup) in lookups.iter().enumerate() {
                ...
                println!("input_polys len: {:?}", input_polys.len());
                ...
                let mut testing_values = domain.empty_extended();
                // Lookup constraints
                parallelize(&mut testing_values, |values, start| {
                    ...
                    for (i, value) in values.iter_mut().enumerate() {
                        ...
                        *value = helper_coset[idx] * product - sum_partial_products;
                    }
                });
                let testing_coeff = domain.extended_to_coeff(testing_values.clone());

                let quotient = domain.divide_by_vanishing_poly(testing_values);
