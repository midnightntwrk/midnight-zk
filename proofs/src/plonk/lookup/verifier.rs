...
impl<F: PrimeField, CS: PolynomialCommitmentScheme<F>> Committed<F, CS> {
    pub(crate) fn evaluate<T: Transcript>(
        self,
        transcript: &mut T,
    ) -> Result<(Committed<F, CS>, lookup::Evaluated<F>), Error>
    where
        F: Hashable<T::Hash>,
    {
...
pub(in crate::plonk) fn queries<'com, F, CS>(
    eval: &'com lookup::Evaluated<F>,
    com: &'com Committed<F, CS>,
    vk: &VerifyingKey<F, CS>,
    x: F,
) -> impl Iterator<Item = VerifierQuery<'com, F, CS>> + Clone
where
    ...
    CS: PolynomialCommitmentScheme<F>,
{
    ...
    iter::empty()
        ...
        .chain(Some(VerifierQuery::new(
            x,
            &com.product_commitment,
            eval.product_eval,
        )))
        // Open lookup input commitments at x
        .chain(Some(VerifierQuery::new(
            x,
            &com.permuted.permuted_input_commitment,
            eval.permuted_input_eval,
        )))
        // Open lookup table commitments at x
        .chain(Some(VerifierQuery::new(
            x,
            &com.permuted.permuted_table_commitment,
