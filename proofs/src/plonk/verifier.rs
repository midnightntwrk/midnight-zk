    ...
    CS::Commitment: Hashable<T::Hash>,
{
    ...
    // corresp. to simple selectors)
    let num_evaluated_fix_queries = vk.cs.num_fixed_columns() - vk.cs.selector_flags.len();
    let mut fixed_evals = read_n(transcript, num_evaluated_fix_queries)?;
