...
>(
    ...
    for _ in 0..h_limb_commitments.len() {
        xn_powers.push(-xn_pow * vanishing_val);
        xn_pow *= xn;
    }
    ...
    // TODO: implement potential speed-up: collect all fully evaluated identities
    // and accumulate them with the correct power of y -> in the end, only a
    // single scalar multiplication is needed for all full evaluations
    for (i, (idx, e)) in expressions.iter().enumerate() {
