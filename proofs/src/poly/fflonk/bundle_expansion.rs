...
//! "fflonk-specific" phase that runs before the standard Halo2 GWC argument:
//!
//! - **Prover (`multi_open`)**: classify queries into bundles, materialise
//!   the combined polynomial `g` for each `t > 1` bundle, compute the
//!   over-opening point-set unions, expand into synthetic queries on `g` at
//!   the `t`-th roots of each logical opening point.
//! - **Verifier (`multi_prepare`)**: classify queries into bundles, collect
//!   the per-(slot, logical) evaluations the prover wrote, reconstruct
//!   `g(root)` for each `t`-th root via Lemma 5.1.
//!
//! The two sides must produce *identical* enumerations of (bundle, slot,
//! point) so the transcript I/O matches. That symmetry is the soundness-
//! critical part of the design — this module exists to make it visible,
//! by holding the data shapes and the deterministic-ordering helpers in
//! one place.
//!
//! The transcript I/O itself stays in `mod.rs` (it needs `&mut T` and
//! interleaves with non-bundle logic like `fewer-point-sets`).
...
use crate::{
    poly::{
        fflonk::FflonkScheme,
        kzg::{compute_dummy_queries, msm::MSMKZG},
...
/// to (a) write over-opening evals to the transcript and (b) emit the
/// synthetic queries on `g`.
...
pub(super) struct BundleAcc<E: MultiMillerLoop> {
    ...
    pub(super) evals: HashMap<(usize, E::Fr), E::Fr>,
}
...
pub(super) fn build_prover_multi_pre<F: PrimeField + Ord + std::hash::Hash>(
    ...
    let mut multi_pre: Vec<MultiPreData<F>> = Vec::new();
    for (bundle_idx, indices) in sub_bundles.iter().enumerate() {
        ...
        let bundle_label_set: HashSet<&PolynomialLabel> = bundle_labels.iter().collect();
        let synth_label = synthetic_bundle_label(&bundle_labels);

        // `(slot, point)` pairs in queries-iter order — same input on both
        // sides ⇒ same `compute_dummy_queries` enumeration ⇒ matching
        // transcript order.
        ...
        let mut union_set: HashSet<F> = HashSet::new();
        for &(_, p) in &pairs {
            union_set.insert(p);
        }
...
/// - `multi_bundles_sorted`: a `Vec` of `(synth_label, BundleAcc)`, sorted
///   by `synth_label.to_string()` so the over-opening read order matches
///   the prover's write order.
...
///   commitments and `Linear`s — passed straight through to GWC.
///
/// Pure function — no transcript I/O. Mirror of `build_prover_multi_pre`
/// for the verifier side.
#[allow(clippy::type_complexity)]
pub(super) fn classify_verifier_queries<'com, E, const T_MAX_LOG: u32>(
...
where
    ...
    FflonkCommitment<E, T_MAX_LOG>: 'com,
{
    let mut multi_bundles: HashMap<PolynomialLabel, BundleAcc<E>> = HashMap::new();
    ...
    for q in queries.iter() {
        ...
        match bundle {
            ...
            FflonkBundle::Bundle(p, labels) => {
                ...
                // the scheme's `T_MAX_LOG`. The trailing bundle may have
...
pub(super) fn synth_triples_for_bundle<E: MultiMillerLoop>(
    ...
    acc: &BundleAcc<E>,
    t_th_root_cache: &mut HashMap<(E::Fr, usize), E::Fr>,
) -> Vec<(PolynomialLabel, E::Fr, E::Fr)>
where
    E::Fr: std::hash::Hash + Eq,
{
    ...
    let mut union_set: HashSet<E::Fr> = HashSet::new();
    for &(_, p) in &acc.pairs {
        union_set.insert(p);
    }
    let mut triples: Vec<(PolynomialLabel, E::Fr, E::Fr)> = Vec::new();
    for logical in union_set {
        // Cache across bundles: `t_th_root(logical, t)` only depends on the
        // pair, but the function is called per (bundle, logical). Multiple
        // bundles typically share the same logical points (ζ, ζ·ω, …), so
        // recomputing the sqrt chain per bundle is gratuitous. Caching by
        // `(logical, t)` cuts those redundant `log2(t)` sqrts.
            ...
            .map(|slot| {
                if slot < real_count {
                    *acc.evals.get(&(slot, logical)).expect(
                    ...
                    // Pad slot — its underlying polynomial is the zero
                    // polynomial, so its eval at any point is zero.
                    E::Fr::ZERO
                }
            })
            .collect();
