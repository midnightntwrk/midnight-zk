...
pub trait Params {
    ...
    /// `whole-h-poly` feature is enabled the monomial basis may be larger than
    /// the Lagrange basis (which covers only the circuit domain), so this
    /// method returns the true capacity for coefficient-form commitments.
