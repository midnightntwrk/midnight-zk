...
impl<E: Engine + Debug> ParamsKZG<E> {
    ...
    ///
    /// Use this when the `whole-h-poly` feature is enabled: generate an SRS
    /// large enough for the whole quotient polynomial (i.e. with `k'` such that
    /// `2^{k'} ≥ (n-1) * quotient_poly_degree`), then call
