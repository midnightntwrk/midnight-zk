...
* `Hashable::read_sized` and `Transcript::read_sized`, for reading a composite value whose item count is supplied by the caller instead of by the proof: the items are read one by one and the assembled value is absorbed once. `read_sized` defaults to `read`, so only a type whose wire form carries no count of its own overrides it [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)

### Fixed
* A commitment group can no longer claim a different number of polynomials than the verifying key expects: the count never travels in the proof, so `read_commitment` builds exactly one inner commitment per label. Previously the points read were zipped against the labels, and a group truncated to one point made every query of that group resolve to that single commitment [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)
...
* `multi_prepare` reads the batch commitment and the opening proof as one-polynomial groups instead of taking their size from the proof, so a malformed proof can no longer reach `KZGMultiCommitment::into_single`'s assertion and panic the verifier [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)
* Cost model: account for the logup polynomials being committed in the two argument phase groups rather than one group per lookup argument. `commit(n)` is now the byte length of the single message that commits to `n` polynomials, so a site writing `n` commitments separately costs `n * commit(1)` [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)
...
* Write a commitment group to the proof as its bare points, with no length framing, and absorb the whole group in one go. How many polynomials a group holds is pinned by the verifying key, so `read_commitment` hands that count to `Transcript::read_sized` rather than reading it back off the wire. `Hashable::read` for `KZGMultiCommitment` is consequently unimplemented, like its `ProcessedSerdeObject::read` counterpart, and `KZGCommitmentScheme` overrides neither `write_commitment` nor `commitment_byte_length`, both of which now exist on the trait purely for schemes that fold polynomials together [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)
* The phase groups of `VerifierTrace` are now optional: a group with no polynomials is not committed to by the prover, so there is nothing to read back. `argument::verifier::Committed::read` asserts that it is given at least one label, and `read_group` handles the empty case for its callers [#515](https://github.com/midnightntwrk/midnight-zk/pull/515)
